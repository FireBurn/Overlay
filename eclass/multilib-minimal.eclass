...
multilib-minimal_pkg_postinst() {
        multilib-minimal_abi_pkg_postinst() {
                pushd "${BUILD_DIR}" >/dev/null || die
                if declare -f multilib_pkg_postinst >/dev/null ; then
        ...
        multilib_foreach_abi multilib-minimal_abi_pkg_postinst
}
