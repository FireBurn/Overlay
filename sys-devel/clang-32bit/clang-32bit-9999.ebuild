...
DEPEND="static-analyzer? ( dev-lang/perl )
	${PYTHON_DEPS}"
RDEPEND="~sys-devel/llvm-32bit-${PV}[debug=,multitarget=,video_cards_radeon=]
	${PYTHON_DEPS}"
...
src_install() {
	...
	mv "${D}"/usr/bin/clang "${D}"/usr/bin/clang32 || die "Removing files failed."
	rm -rf "${D}"/usr/share || die "Removing files failed."
	rm -rf "${D}"/usr/include || die "Removing files failed."
	rm -rf "${D}"/usr/bin/asan* || die "Removing files failed."
	rm -rf "${D}"/usr/bin/c++-analyzer || die "Removing files failed."
	rm -rf "${D}"/usr/bin/c-index-test || die "Removing files failed."
	rm -rf "${D}"/usr/bin/ccc-analyzer || die "Removing files failed."
	rm -rf "${D}"/usr/bin/clang++ || die "Removing files failed."
	rm -rf "${D}"/usr/bin/clang-* || die "Removing files failed."
	rm -rf "${D}"/usr/bin/scan-* || die "Removing files failed."
}
