...
src_configure() {
	...
	local mycmakeargs=(
		...
		# the AOT kernel cache build initializes the HIP runtime and
		# violates the sandbox on /dev/kfd; kernels are compiled at
		# runtime via hiprtc and cached in the user's home instead
...
src_install() {
	cmake_src_install
	rm -r "${ED}/usr/share/doc/rocfft" || die
	dodoc LICENSE.md
