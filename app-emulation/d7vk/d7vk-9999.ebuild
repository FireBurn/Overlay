...
if [[ ${PV} == 9999 ]]; then
	...
	# Note: d7vk release tarballs with submodules would need to be hosted somewhere
	SRC_URI="https://github.com/WinterSnowfall/d7vk/releases/download/v${PV}/${P}.tar.gz"
fi

KEYWORDS="-* ~amd64 ~x86"

DESCRIPTION="Vulkan-based implementation of DirectDraw and D3D7 for Linux / Wine"
...
SLOT="0"
IUSE="+abi_x86_32 crossdev-mingw +d3d8 +strip"
BDEPEND="
	${PYTHON_DEPS}
	dev-util/glslang
...
src_prepare() {
	...
	mv setup_dxvk.sh setup_d7vk.sh || die
	sed -i "/^basedir=/s|=.*|=${EPREFIX}/usr/lib/${PN}|" setup_d7vk.sh || die
	
	# Strip dxgi, d3d9, d3d10, d3d11 from the setup script
	sed -i -e '/if \$with_dxgi/,/fi/d' -e '/\$action d3d9/d' -e '/\$action d3d10core/d' -e '/\$action d3d11/d' setup_d7vk.sh || die
	sed -i 's/ \[--without-dxgi\]//g' setup_d7vk.sh || die
	
	echo "\$action ddraw" >> setup_d7vk.sh
	if use d3d8; then
		echo "\$action d3d8" >> setup_d7vk.sh
	fi
}
...
pkg_postinst() {
	if [[ ! ${REPLACING_VERSIONS} ]]; then
		...
		elog "Note: setup_d7vk.sh is unofficially temporarily provided as it was"
