...
DEPEND=">=media-libs/libsdl-1.2.10[${MULTILIB_USEDEP}]
	flac? ( media-libs/flac[${MULTILIB_USEDEP}] )
	midi? (
		fluidsynth? ( media-sound/fluidsynth[${MULTILIB_USEDEP}] )
		timidity? ( media-sound/timidity++[${MULTILIB_USEDEP}] )
	)
	mp3? (
	...
	vorbis? ( >=media-libs/libvorbis-1.0_beta4[${MULTILIB_USEDEP}] media-libs/libogg[${MULTILIB_USEDEP}] )"
RDEPEND=${DEPEND}
