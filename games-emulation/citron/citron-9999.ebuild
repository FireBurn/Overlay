...
EGIT_SUBMODULES=( '-*' 'dynarmic' 'simpleini' 'sirit' 'tzdb_to_nx' 'externals/nx_tzdb/tzdb_to_nx/externals/tz/tz' 'externals/sirit/externals/SPIRV-Headers'  'VulkanMemoryAllocator' 'xbyak' )
...
src_configure() {
	local -a mycmakeargs=(
		...
		-DCITRON_USE_EXTERNAL_VULKAN_HEADERS=$(usex system-vulkan no yes)
		-DSIRIT_USE_SYSTEM_SPIRV_HEADERS=$(usex system-vulkan no yes)
		-DCITRON_USE_EXTERNAL_VULKAN_UTILITY_LIBRARIES=$(usex system-vulkan no yes)
		-DCITRON_USE_EXTERNAL_VULKAN_SPIRV_TOOLS=$(usex system-vulkan no yes)
		-DCITRON_USE_EXTERNAL_SDL2=OFF
		-DCITRON_USE_QT_WEB_ENGINE=$(usex webengine)
		-DCITRON_CMD=OFF
		-DCITRON_ROOM=OFF
	)
