...
inherit autotools libtool flag-o-matic eutils portability multilib-minimal
...
multilib_src_install() {
	...
	if multilib_is_native_abi; then
		# Move files back.
		if path_exists -o "${ED}"/tmp/glib-config.*; then
			mv "${ED}"/tmp/glib-config.* "${ED}"/usr/bin || die
		fi
	else
		# Preserve ABI-variant of glib-config,
		# then drop all the executables
		mkdir -p "${ED}"/tmp || die
		mv "${ED}"/usr/bin/glib-config "${ED}"/tmp/glib-config.${ABI} || die
		rm -r "${ED}"/usr/bin || die
	fi
}
